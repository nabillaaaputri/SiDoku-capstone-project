        ...
        <div className="space-y-3 p-4 sm:p-4.5 lg:p-5">
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            {Array.from({ length: 2 }).map((_, index) => (
              <div key={`forecast-skeleton-${index}`} className="h-16 rounded-2xl bg-slate-100 animate-pulse" />
            ))}
          </div>
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_10px_30px_rgba(15,23,42,0.06)]">
      ...
      <div className="space-y-4 p-4 sm:p-4.5 lg:p-5">
        ...
        <ChartContainer config={chartConfig} className="aspect-auto h-[320px] w-full rounded-[24px] border border-slate-200/60 bg-[linear-gradient(180deg,_#ffffff,_#f8fafc)] p-3 sm:p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.75),0_4px_24px_rgba(15,23,42,0.04)]">
