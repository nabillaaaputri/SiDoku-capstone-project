...
export default function StockOut() {
  ...
  const selectedProduct = activeProducts.find((p) => p.id === stockOutForm.productId);
  ...
  const handleSelectProduct = (productId: string) => {
    ...
    if (!product) {
      toast({
        ...
        description: "Produk yang diarsipkan tidak bisa dipilih untuk stok keluar.",
        variant: "destructive",
      });
      return;
    }
  ...
  const handleStockOutSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const product = activeProducts.find((p) => p.id === stockOutForm.productId);
      if (!product) {
        toast({
          title: "Produk tidak tersedia",
          description: "Produk ini sudah diarsipkan dan tidak bisa digunakan untuk stok keluar.",
          variant: "destructive",
        });
        return;
      }
                    ...
                    <tbody>
                      {filteredHistoryStockOuts.map((item) => (
                        <tr key={item.id} className="border-b border-slate-100 last:border-b-0 hover:bg-slate-50/60">
                          ...
                          <td className="px-5 py-4 align-top text-center text-sm font-semibold text-red-600">-{item.quantity}</td>
                    ...
                    <div key={item.id} className="rounded-xl border border-slate-100 bg-white p-3">
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <div className="text-right">
                          <p className="text-sm font-semibold text-red-600">-{item.quantity}</p>
